...
async def analyze_mention(text: str, url: str, search_term: str,
    ...
    prompt = f"""Проанализируй, является ли этот текст содержательным упоминанием компании в медиа.

ПОИСКОВЫЙ ЗАПРОС: «{search_term}»
URL: {url}
ТЕКСТ: {text[:2000]}

Задача 1 — РЕЛЕВАНТНОСТЬ (is_relevant: true/false).
Упоминание НЕРЕЛЕВАНТНО (false) если выполняется ХОТЯ БЫ ОДНО из условий:
- Компания «{search_term}» просто числится в списке клиентов, партнёров, контрагентов или портфолио — без конкретной новости или события
- Аббревиатура (ООО, ЗАО, АО) совпала случайно с другой организацией
- Страница — справочник юрлиц (audit-it.ru, rusprofile.ru, focus.kontur.ru и т.п.)
- Упоминание на TikTok/VK/OK без явной связи с брендом «{search_term}»
- Текст вообще не содержит словосочетание «{search_term}» целиком
- Страница — вакансии, резюме или учебные/академические работы без новостного контекста

Упоминание РЕЛЕВАНТНО (true) если:
- Есть конкретное событие, новость, отзыв, кейс или обсуждение именно этой компании/продукта
- Компания — один из главных субъектов текста, а не просто элемент списка

Задача 2 — ТОНАЛЬНОСТЬ (только если is_relevant: true):
- positive — похвала, успех, награда, рост
- negative — критика, жалоба, проблема, скандал
- neutral — нейтральные факты без оценки

Задача 3 — РЕЗЮМЕ (только если is_relevant: true): суть события, до 150 символов, по-русски.
Не начинай с «В тексте», «Статья», «Автор».
